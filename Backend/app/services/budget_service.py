...
class BudgetService:
    ...
    def _sync_goal_current_amounts(self, user_id: int):
        """Synchronize each goal's `current_amount` field (cache on savings_goals)
        with the authoritative sum of its allocations. Also deactivates goals
        that have reached or exceeded their target.
        """
    ...
    def _compute_budget_state(self, budget: BudgetPlan, user_id: int):
        ...
        self._sync_goal_current_amounts(user_id=user_id)
    ...
    def refresh_goal_state_for_user(self, user_id: int, preferred_budget_id: int | None = None, commit: bool = True):
        # `preferred_budget_id` is kept in the signature for backward compatibility
        # with existing callers, but no longer needed (goal state is derived live
        # from allocations and synchronized onto `savings_goals.current_amount`).
        del preferred_budget_id
        self._sync_goal_current_amounts(user_id=user_id)
